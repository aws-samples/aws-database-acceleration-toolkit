locals {
  ...
  internal_db_subnet_group_name = try(coalesce(var.db_subnet_group_name, "${var.name}-dbsubnetgroup-${var.environment}-${random_id.this.hex}"), "")
  master_password               = var.create_cluster && var.create_random_password ? random_password.master_password[0].result : jsondecode(data.aws_secretsmanager_secret_version.get-version[0].secret_string)["password"]
...
resource "aws_rds_cluster" "this" {
  ...
  cluster_identifier             = "${var.name}-cluster-${var.environment}-${random_id.this.hex}"
  ...
  final_snapshot_identifier           = "${var.final_snapshot_identifier_prefix}-${var.name}-${var.environment}-${element(concat(random_id.snapshot_identifier.*.hex, [""]), 0)}"
  ...
  db_cluster_parameter_group_name     = var.db_cluster_parameter_group_name == "" ? aws_rds_cluster_parameter_group.this[0].name : var.db_cluster_parameter_group_name
...
resource "aws_rds_cluster_instance" "this" {
  ...
  identifier                            = var.instances_use_identifier_prefix ? null : lookup(each.value, "identifier", "${var.name}-${var.environment}-${random_id.this.hex}-${each.key}")
  identifier_prefix                     = var.instances_use_identifier_prefix ? lookup(each.value, "identifier_prefix", "${var.name}-${var.environment}-${random_id.this.hex}-${each.key}-") : null
  ...
  db_subnet_group_name                  = local.db_subnet_group_name
  db_parameter_group_name               = var.db_parameter_group_name != "" ? lookup(each.value, "db_parameter_group_name", var.db_parameter_group_name) : aws_db_parameter_group.this[0].name
  ...
  performance_insights_kms_key_id       = var.performance_insights_kms_key_id != "" ? lookup(each.value, "performance_insights_kms_key_id", var.performance_insights_kms_key_id) : aws_kms_key.this[0].arn
